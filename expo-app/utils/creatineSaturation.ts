   ...
   logs.forEach((entry, i) => {
      ...
      if (i === 0) {
         ...
         if (dose > 0) {
            ...
            const effectiveDose = isLoadingPhase ? Math.min(dose, 20) / 20 : Math.min(dose, 5) / 5;

            saturations[i] = Math.min(prev + rate * effectiveDose * (1 - prev), 1);
         } else {
...
export const calculateDaysTillSaturated = (
   ...
   while (saturation < TARGET_SATURATION && days < 30) {
      ...
      if (plannedDailyDose > 0) {
         saturation = Math.min(saturation + rate * effectiveDose * (1 - saturation), 1);
      }
   }
