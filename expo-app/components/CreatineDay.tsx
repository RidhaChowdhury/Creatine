...
const CreatineDay = (props: any) => {
   ...
   const handleDelete = async (id: string) => {
      const { error } = await supabase.from('creatine_logs').delete().eq('id', id);
      if (error) {
      ...
      <View>
         <View className='flex-row items-center justify-between p-4'>
            <Text className='text-[20px] font-bold text-white'>
               ...
               {data.reduce((total, log) => total + log.dose_grams, 0) === 1 ? 'gram' : 'grams'}
            </Text>
            <Text className='text-[20px] font-bold text-white'>
         ...
         <View className='mb-[10] px-[15]'>
            {data.length === 0 && (
               <Text className='text-white ml-4 mb-[10]'>No creatine logs for this day...</Text>
            )}
            {data.map((log) => (
               <View
                  key={log.id}
