...
import { Canvas, Path, Skia, useClock, vec, LinearGradient } from '@shopify/react-native-skia';
...
export const WaveBackground = ({
	...
	const basePosition = useDerivedValue(() => {
		if (progressPercent == 0) return height;
	...
	// Gradient positions
	const gradientStart = useDerivedValue(() => vec(0, basePosition.value - amplitude));
	const gradientEnd = useDerivedValue(() => vec(0, basePosition.value + height * 0.5));

	return (
		<Canvas style={{ position: 'absolute', width, height }}>
			<Path path={path}>
				<LinearGradient start={gradientStart} end={gradientEnd} colors={colors} />
			</Path>
		</Canvas>
	);
};
